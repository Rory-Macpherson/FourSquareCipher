...
public class FourSquareCipher {
   ...
   // cipherrunner calls these, and sends them text, then they in turn call and return process
    ...
    // First it puts the key letters (no repeats), then fills in the rest of the alphabet
    ...
    private String keyBuilder(String key) {
        ...
        for (int i = 0; i < key.length(); i++) {
            char c = key.charAt(i);
            if (result.toString().indexOf(c) == -1) {//this is just searching the string for 'c'. if there was 1 in there it would come back as 1 and therefore not enter it again
        ...
        for (char c = 'A'; c <= 'Z'; c++) {//its a simple loop statement
            if (c == 'J') continue; //if j is in there, just skip the rest of the for loop
    ...
    private String process(String text, boolean encryptMode) {
        ...
        if (encryptMode) {
            paddingAdded = false;
        } //this resets the padding added. its a little messy this removing the x but i hate the extra x
        ...
        System.out.println("\nProcessing text: " + clean + " (" + clean.length() + " letters)");
     ...
     // If we are decrypting AND the switch is ON
        ...
        // This part puts the spaces and punctuation back where they were in the original text
    ...
    // Parameters: a and b are the two letters to transform, its a lot and a bit messy, but works so far
    // encryptMode: true = encrypt, false = decrypt, when you call it you have to also feed in a boolean
        ...
        if (encryptMode) {
            ...
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    ...
                    if (TLeft[i][j] == a) { row1 = i; col1 = j; }
                    // Find where 'b' is located in the BRight grid
                    if (BRight[i][j] == b) { row2 = i; col2 = j; }
                }
            }
            // The encrypted pair is formed by:
            // Taking the letter from TRight at (rowA, colB)
            // Taking the letter from BLeft at (rowB, colA)
            return "" + TRight[row1][col2] + BLeft[row2][col1];// you need to add the "" otherwise the thing crashes, it forces it to be a string.
            //when we return it we mix the rows and cols. confusing but once you keep the same pattern it works out
        } else {
            // Decrypt
            ...
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    ...
                    if (TRight[i][j] == a) { row1 = i; col1 = j; }
                    // Find where b is in the BLeft grid 
                    if (BLeft[i][j] == b)   { row2 = i; col2 = j; }
                }
            }
    ...
    private void printSquare(char[][] square, String name) {
        System.out.println("\n" + name);// \n means pressing the enter button or new line 
        System.out.println("-----------------");
